...
run.SVM = function(svm_type = 'C-svc',
    ...
    png(paste0('./Figures/SVM_',config,'_', gsub('/', '-', off), '_', TR, '_',season, yr_test, '_C', as.character(c_value), '_', ker, '_', kp, as.character(eps),'.png'),
    ...
    text(x = -77.12, y = 38.833, labels = paste('Epsilon = ', eps), cex = 1.3, pos = 4)
    text(x = -77.12, y = 38.823, labels = paste('C = ', c_value), cex = 1.3, pos = 4)
    text(x = -77.12, y = 38.813, labels = paste('Kernel = ', ker), cex = 1.3, pos = 4)
    text(x = -77.12, y = 38.803, labels = paste('Sigma = ', kp), cex = 1.3, pos = 4)
    ...
    png(paste0('./Figures/SVM_',config,'_', gsub('/', '-', off), '_', TR, '_',season, yr_test, '_C', as.character(c_value), '_', ker, '_', kp,as.character(eps),'_BAR.png'),
        width = 1200, height = 500)
    tb_pre = table(pltClass_res)
    names(tb_pre) = c('Below Median', 'Above Median','Errors')
    par(mfrow = c(1,2))
    barplot(tb_test,col=c(blue,red),
            legend = NULL, beside=TRUE, border = 'white', cex.names = 1.5, ylim = c(0,90))
    ...
    names(output) = c('Error', 'Below Median', 'Above Median', 'Errors')
